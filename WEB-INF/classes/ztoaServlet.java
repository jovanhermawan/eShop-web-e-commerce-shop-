...
@WebServlet("/ztoa")   // Configure the request URL for this servlet (Tomcat 7/Servlet 3.0 upwards)
public class ztoaServlet extends HttpServlet {
   ...
   public void doGet(HttpServletRequest request, HttpServletResponse response)
      ...
      try (
         ...
         String sqlStr = "SELECT items.id, items.name, items.stock, items.price, items.type, items.imgurl, rating.rating, rating.voted_user FROM items, rating where stock > 0 and items.id = rating.id ORDER BY name DESC";
 // Echo for debugging
         ...
         sqlStr = "SELECT items.id, items.name, items.stock, items.price, items.type, items.imgurl, rating.rating, rating.voted_user FROM items, rating where stock <= 0 and items.id = rating.id ORDER BY price ASC";
 // Echo for debugging
