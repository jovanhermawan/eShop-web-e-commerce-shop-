...
public class searchServlet extends HttpServlet {
   ...
   public void doGet(HttpServletRequest request, HttpServletResponse response)
      ...
      try (
         ...
         String sqlStr = "SELECT items.id, items.name, items.stock, items.price, items.type, items.imgurl, rating.rating, rating.voted_user FROM items, rating where stock > 0 and items.id = rating.id and name like '%"+search+"%' ORDER BY name asc";
 // Echo for debugging
         ...
         sqlStr = "SELECT items.id, items.name, items.stock, items.price, items.type, items.imgurl, rating.rating, rating.voted_user FROM items, rating where stock <= 0 and items.id = rating.id and name like '%"+search+"%' ORDER BY rating DESC";
 // Echo for debugging
